...
function createChannelsStore() {
  ...
  return {
    ...
    fetchChannel: async (chatbotId: number) => {
      ...
      const API_URL = `https://kibot.azurewebsites.net/api/v1/chatbots/${chatbotId}/channels`;
    ...
    updateChannel: async (chatbotId: number, channelId: number, channel: Channel) => {
      update((state) => ({ ...state, loading: true }));
      const API_URL = `https://kibot.azurewebsites.net/api/v1/chatbots/${chatbotId}/channels/${channelId}`;
